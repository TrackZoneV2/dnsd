...
State.prototype.record = function (section_name, record) {
    ...
    if (section_name != "question") {
        ...
        switch (record.class + " " + record.type) {
            ...
            case "IN CAA":
                rdata = [
                    new Buffer([record.data.flags]),
                    self.encode(record.data.tag),
                    new Buffer(record.data.value, "utf8"),
                ];
                break;
            case "NONE A":
            ...
            default:
                throw new Error(
                    "Unsupported record type: " + JSON.stringify(record)
                );
        }
...
function pair_to_buf(pair) {
    ...
    if (!pair.match(/^[0-9a-fA-F]{4}$/))
        throw new Error(
            "Bad " + record.type + " record data: " + JSON.stringify(record)
        );
